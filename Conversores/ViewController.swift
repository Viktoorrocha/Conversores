//
//  ViewController.swift
//  Conversores
//
//  Created by Viktor Rocha on 30/09/2018.
//  Copyright © 2018 Viktor Rocha. All rights reserved.
//

import UIKit

class ViewController: UIViewController {

    @IBOutlet weak var tfValor: UITextField!
    @IBOutlet weak var lbResultUnit: UILabel!
    @IBOutlet weak var lbResultado: UILabel!
    @IBOutlet weak var btUnit2: UIButton!
    @IBOutlet weak var btUnit1: UIButton!
    @IBOutlet weak var lbUnit: UILabel!
    
    
    override func viewDidLoad() {
        super.viewDidLoad()
        // Do any additional setup after loading the view, typically from a nib.
    }


    @IBAction func showNext(_ sender: UIButton) {
        switch lbUnit.text!{
        case "Temperatura":
            lbUnit.text = "Peso"
            btUnit1.setTitle("Kilograma", for: .normal)
            btUnit2.setTitle("Libra", for: .normal)
        case "Peso":
            lbUnit.text = "Moeda"
            btUnit1.setTitle("Real", for: .normal)
            btUnit2.setTitle("Dólar", for: .normal)
        case "Moeda":
            lbUnit.text = "Distância"
            btUnit1.setTitle("Metro", for: .normal)
            btUnit2.setTitle("Kilômetro", for: .normal)
        default :
            lbUnit.text = "Temperatura"
            btUnit1.setTitle("Celsius", for: .normal)
            btUnit2.setTitle("Farenheint", for: .normal)
        }
        
        convert(nil)
    }
    
    @IBAction func convert(_ sender: UIButton?) {
        if let sender = sender {
            if sender  == btUnit1 {
                btUnit2.alpha = 0.5
            } else {
                btUnit1.alpha = 0.5
            }
            sender.alpha = 1.0
        }
        
        switch lbUnit.text!{
        case "Temperatura":
            calcTemperature()
        case "Peso":
            calcWeight()
        case "Moeda":
            calcCurrency()
        default:
            calcDistance()
        }
        view.endEditing(true)
        let result = Double(lbResultado.text!)!
        lbResultado.text = String(format: "%.2f", result)
        
    }
    
    func calcTemperature(){
        
        guard let temperature = Double(tfValor.text!) else {return}
        
        if btUnit1.alpha == 1.0{
            lbResultUnit.text = "Farenheint"
            lbResultado.text = String(temperature * 1.8 + 32.0)
        } else {
            lbResultUnit.text = "Celsius"
            lbResultado.text = String((temperature - 32.0) / 1.8)
        }
        
    }
    
    
    func calcWeight(){
        guard let weight = Double(tfValor.text!) else {return}
        if btUnit1.alpha == 1.0{
            lbResultUnit.text = "Libra"
            lbResultado.text = String(weight / 2.2046 )
        } else {
            lbResultUnit.text = "Kilograma"
            lbResultado.text = String( weight * 2.2046 )
        }
       
        
    }
    
    func calcCurrency(){
        guard let currency = Double(tfValor.text!) else {return}
        if btUnit1.alpha == 1.0{
            lbResultUnit.text = "Dólar"
            lbResultado.text = String(currency / 3.5)
        } else {
            lbResultUnit.text = "Real"
            lbResultado.text = String(currency * 3.5)
        }
        
        
    }
    
    func calcDistance(){
        
        guard let distance = Double(tfValor.text!) else {return}
        if btUnit1.alpha == 1.0{
            lbResultUnit.text = "Kilômetro"
            lbResultado.text = String(distance / 1000.0)
        } else {
            lbResultUnit.text = "Metros"
            lbResultado.text = String(distance * 1000.0)
        }
        
    }
    
    
}

